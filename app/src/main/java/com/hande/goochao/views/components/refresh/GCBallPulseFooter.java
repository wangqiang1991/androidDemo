...
public class GCBallPulseFooter extends ViewGroup implements RefreshFooter {

    private GCBallPulseView mBallPulseView;
    private SpinnerStyle mSpinnerStyle = SpinnerStyle.Translate;
    private Integer mNormalColor;
    private Integer mAnimationColor;

    //<editor-fold desc="ViewGroup">
    public GCBallPulseFooter(@NonNull Context context) {
        super(context);
        initView(context, null, 0);
    }

    public GCBallPulseFooter(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initView(context, attrs, 0);
    }

    public GCBallPulseFooter(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initView(context, attrs, defStyleAttr);
    }

    private void initView(Context context, AttributeSet attrs, int defStyleAttr) {
        mBallPulseView = new GCBallPulseView(context);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // 保存测量后的大小，+ DensityUtil.dp2px(15) 代表在原始高度上再加15dp高度。YS二次修改
        setMeasuredDimension(
                resolveSize(mBallPulseView.getMeasuredWidth(), widthMeasureSpec),
                resolveSize(mBallPulseView.getMeasuredHeight(), heightMeasureSpec) + DensityUtil.dp2px(15)
        );
    }
    ...
    public GCBallPulseFooter setSpinnerStyle(SpinnerStyle mSpinnerStyle) {
        this.mSpinnerStyle = mSpinnerStyle;
        return this;
    }

    public GCBallPulseFooter setIndicatorColor(@ColorInt int color) {
        mBallPulseView.setIndicatorColor(color);
        return this;
    }

    public GCBallPulseFooter setNormalColor(@ColorInt int color) {
        mNormalColor = color;
        mBallPulseView.setNormalColor(color);
        return this;
    }

    public GCBallPulseFooter setAnimatingColor(@ColorInt int color) {
